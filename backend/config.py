...
_ENV_BASELINE = {k: v for k, v in os.environ.items() if k.startswith(("SEARXNG_", "SEARCH_", "KEENABLE_", "BRAVE_", "TAVILY_", "AI_", "DEEPSEEK_", "QWEN_", "HTTP_", "MAX_"))}
...
SECRET_KEYS = (
    ...
    "KEENABLE_API_KEY", "BRAVE_API_KEY", "TAVILY_API_KEY",
)
...
def _read_all():
    ...
    # 但不再让部署配置改变 SearXNG → Keenable → Brave → Tavily 的产品规则。
    SEARCH_PROVIDER_ORDER = ["searxng", "keenable", "brave", "tavily"]
    ...
    KEENABLE_API_KEY = os.getenv("KEENABLE_API_KEY", "").strip()
    KEENABLE_SEARCH_ENDPOINT = os.getenv(
        "KEENABLE_SEARCH_ENDPOINT", "https://api.keenable.ai/v1/search"
    ).strip().rstrip("/")
    KEENABLE_SNIPPET_MAX_LENGTH = max(
        180, min(10000, int(os.getenv("KEENABLE_SNIPPET_MAX_LENGTH", "1200")))
    )
