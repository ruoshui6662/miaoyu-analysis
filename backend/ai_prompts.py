...
SYSTEM_BASE = (
    ...
    "协议要求：只输出一个 JSON 对象；禁止输出 <think> 推理标签、Markdown 代码围栏、前后解释文字或其他字符。"
...
def expand_keywords_prompt(topic: str) -> list[dict]:
    return [
        {"role": "system", "content": (
            ...
            '只输出 JSON：{"keywords": ["..."至少8个关键词]}；禁止输出 <think>、Markdown 围栏或解释文字。'
        )},
...
def facts_prompt(topic: str, items: list[dict]) -> list[dict]:
    return [
        {"role": "system", "content": (
            ...
            ' "media_quotes": [{"media": "媒体名", "title": "评论文章标题", "stance": "positive或negative或neutral", "core_view": "该评论的核心观点/立场（40-90字）", "url": "原文链接"}]'
            '（选2-3篇：必须是权威媒体(S/A级)发表的评论性文章——社论/评论/观察/时评/观点栏目，区别于新闻稿；摘取其明确立场与核心论点）,'
            ' "netizen_quotes": [{"platform": "B站/知乎/微博/公众号等", "stance": "positive或negative或neutral", "text": "代表性网友原声（15-60字，保留原意与语气）", "source": "来源平台/账号", "url": "出处链接"}]'
            '（选2-3条，覆盖正/负/中立各至少1：必须是社区互动中的有理有据表达——评论/回帖/自媒体短评，有理据、有观点；排除纯玩梗、表情、情绪宣泄；标注平台与出处）}'
            "媒体观点与网友观点互为补充：媒体观点代表机构立场（审校过的论据），网友观点代表大众情绪（高赞共鸣样本），后者仅作舆论观察不作事实依据。"
            "硬性要求：media_quotes 与 netizen_quotes 每项必须各给出 2-3 条，禁止输出空数组；"
            "若完全找不到权威媒体评论文章，可用素材中观点最鲜明、立场最清晰的权威媒体报道替代并在 media 字段注明“（报道立场）”；"
            "网友观点优先从 B站/知乎/微博/公众号等社区含量高的内容中找；找不到时从素材中最接近大众原声的内容选，并在 source 字段如实标注平台。"
