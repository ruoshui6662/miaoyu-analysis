...
1. **产品品牌标识**：用于妙舆自身的导航栏、登录入口和浏览器标签页，使用 `frontend/assets/miaoyu-logo.svg` 及其 inline SVG 版本。它是红色倾斜圆角框与低对比度圆心组成的连续母形，不得用于任何第三方来源。
2. **功能图标**：导航、展开、删除、分类等界面动作，继续使用项目内的 24×24 单色线性 SVG，统一 `stroke-width: 1.8`、圆角端点和现有设计 token。
3. **来源平台 Logo**：仅用于识别来源平台，使用 Simple Icons 已收录的官方品牌 SVG；保持原始几何比例，在来源行统一放入 27×27 图标槽位，颜色由界面 token 控制。

## 妙舆产品 Logo（C 方案）

正式资产为 `frontend/assets/miaoyu-logo.svg`。C 方案保留“圆角框 + 圆心”的识别骨架，以连续曲线表达信息聚合与观察视角；轻微倾斜只作为识别动作，不延伸为装饰背景。产品文字标识使用“妙”红、“舆”墨色的双色字标。

使用规则：

- 导航栏使用 34×34 的 inline SVG，登录卡片使用 48×48；不得拉伸、裁切或叠加阴影。
- 浏览器 favicon 使用同一 SVG 文件，保持透明背景，以便在浅色和深色浏览器界面中都能识别。
- 品牌红统一引用界面 `--accent`；独立 SVG 因为无法继承 CSS token，固定使用 `#E53935`。
- 产品 Logo 与微博、知乎、B 站等来源 Logo 必须使用不同的组件和数据入口；来源 Logo 继续由 `source_id` 映射。
...
- 普通功能图标统一使用 `--icon-md: 16px`；紧凑刷新控件使用 `--icon-sm: 14px`。报告下载/编辑操作统一 40px 高、16px 图标和 8px 图标间距，移动端按钮高度提升到 44px。
