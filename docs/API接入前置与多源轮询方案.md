...
前端采用“升温主叙事 + 六张同构卡片”的信息层级：主叙事负责引导研判，卡片负责平台横向比较。桌面为 3×2，平板为 2 列，手机为 1 列；每张卡最多加载 15 条，卡内纵向滚动且隐藏滚动条，点击条目打开原文。热度展示遵循 NewsNow 的“来源信息优先”原则：`hot` 有值就原样显示；无值则显示 `heat.label` 的“高热/中热/在榜”，并可通过 `heat.tooltip` 解释为榜内相对位置，不用破折号制造“没有热度”的空白。空榜仍不伪造条目。前端只访问本站接口，部署到其他环境时不依赖浏览器跨域权限。
...
```json
{
  ...
  "heat": {
    ...
    "board_size": 15,
    "raw": "545 万热度",
    "tooltip": "来源热度：545 万热度 · 当前榜内第 7/15"
  }
}
```

没有来源热度时，`basis=rank`，`label` 为“高热/中热/在榜”；`relative_score` 按榜内位置计算（前 20%/前 60%/其余），仅用于同一榜单的相对表达和跨平台的保守排序。禁止把不同平台的原始热度直接相减、相除或合并成未经说明的“全网热度”。
