...
前端采用“升温主叙事 + 六张同构卡片”的信息层级：主叙事负责引导研判，卡片负责平台横向比较。桌面为 3×2，平板为 2 列，手机为 1 列；每张卡最多加载 15 条，卡内纵向滚动且隐藏滚动条，点击条目打开原文。热度展示统一使用 `heat.label` 的“高热/中热/在榜”，不因知乎等来源提供原生数值而切换评价体系；`hot` 与 `heat.raw` 仅作为来源追溯信息，通过 `heat.tooltip` 辅助说明榜内位置，不用破折号制造“没有热度”的空白。空榜仍不伪造条目。前端只访问本站接口，部署到其他环境时不依赖浏览器跨域权限。

### 7.2 统一热度标注契约

`/api/hot/boards` 保留来源原始字段 `hot`，同时为每个条目返回：
...
```json
{
  ...
  "heat": {
    ...
    "board_size": 15,
    "raw": "545 万热度",
    "tooltip": "统一榜内热度：中热 · 第 7/15（来源原生热度：545 万热度，仅作参考）"
  }
}
```

`basis` 固定为 `rank`，`label` 统一为“高热/中热/在榜”；`relative_score` 按榜内位置计算（前 20%/前 60%/其余），仅用于同一榜单的相对表达和跨平台的保守排序。来源若提供原生热度，保留在 `raw` 供追溯，但不改变主显示标签，也不参与跨平台绝对比较。禁止把不同平台的原始热度直接相减、相除或合并成未经说明的“全网热度”。
