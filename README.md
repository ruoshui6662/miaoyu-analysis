...
相关文档：[部署指南 docs/DEPLOY.md](docs/DEPLOY.md) ｜ [开发手册与任务清单 docs/开发手册.md](docs/开发手册.md)
...
配置可视化（A3/A4）→ 历史持久化（B3）→ 报告可视化/评语卡片（B1/B2）→ 信源开关与热榜监测（C1/C2）→ 定时任务（C3）→ 事实验证增强（D）→ 安全基线（E）→ 多模板打磨（F）